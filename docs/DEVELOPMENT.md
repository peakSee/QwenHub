# QwenHub Development Guide

A practical guide for developers working on QwenHub. Covers the codebase structure, development workflow, and common tasks.
...
QwenHub uses **Bun's built-in test runner** (`bun test`) with Bun's assertion library. No external test framework is needed.
