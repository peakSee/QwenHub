# QwenHub API Reference

Complete API documentation for QwenHub's OpenAI-compatible endpoints.
...
QwenHub implements rate limiting to protect Qwen accounts:
...
QwenHub tracks tool call nesting depth during streaming (`toolCallDepth` counter). Content inside tool call XML blocks (`<function=...>`) is suppressed from client emission to prevent chunk-boundary fragments from leaking. The full clean tool call text is delivered as a single `finish_reason: tool_calls` phase.
...
- **Issues**: [GitHub Issues](https://github.com/youssefvdel/qwenhub/issues)
- **Discussions**: [GitHub Discussions](https://github.com/youssefvdel/qwenhub/discussions)
