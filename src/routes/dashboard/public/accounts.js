...
function makeThrottleBadge(acct) {
  if (acct.throttled) {
    ...
    if (acct.throttledUnlockAt) {
      ...
      var timeStr = unlockTime.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false });
      label += ' 至 ' + timeStr;
  ...
  (async function () {
    try {
      ...
      if (result.loginSucceeded) {
        showToast('账户添加并登录成功: ' + email, 'success');
        pollAuth(email, 15);
      } else {
        showToast(result.loginError || '账户已添加但登录失败，请点「登录」打开浏览器。', 'warning');
        pollAuth(email, 15);
      }
      loadAccounts();
    } catch (e) {
  ...
  (async function () {
    try {
      ...
      if (!pwRes.ok || !pwData || !pwData.password) {
        throw new Error(pwData && pwData.error && pwData.error.message ? pwData.error.message : '无法获取密码');
      }
    ...
    .then(function (data) {
      ...
      ws.onmessage = function (evt) {
        ...
        if (msg.type === 'frame') {
          ...
          session.statusText = '实时 — ' + (msg.width || 1280) + 'x' + (msg.height || 800);
...
function closeBrowserTab(idx) {
  ...
  if (browserSessions.length === 0) {
    ...
    document.getElementById('browserStatusInline').textContent = '请选择一个标签';
