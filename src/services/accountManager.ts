...
function deriveKey(keyMaterial: string): Buffer {
  return crypto.scryptSync(keyMaterial, 'qwenhub-salt', 32);
}
...
export function loadAccountsFromFile(): Array<{ email: string; password: string; throttledUntil?: number; disabled?: boolean; proxy?: string }> {
  const tryLoad = (filePath: string): Array<{ email: string; password: string; throttledUntil?: number; disabled?: boolean; proxy?: string }> | null => {
    try {
...
export async function addAccount(email: string, password: string, proxy?: string): Promise<{ loginSucceeded: boolean; loginError?: string }> {
  ...
  const entry: AccountEntry = {
    ...
    totalRequests: 0,
    disabled: false,
    ...(proxy && proxy.trim() ? { proxy: proxy.trim() } : {}),
  };
  accounts.push(entry);
  rebuildEmailIndex();
...
export async function getTokenWithAccount(email?: string): Promise<{ token: string; email: string; proxy: string | null } | null> {
  ...
  return { token: acct.state.token, email: acct.email, proxy: acct.proxy || null };
}
