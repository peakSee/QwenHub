...
export function updateClaudeCodeSettings(cfg: ConfigSchema): void {
  ...
  if (enabled) {
    ...
    const settings = {
      _comment: 'Managed by qwenhub — CLAUDE_CODE_PROXY toggle in dashboard',
      env: {
    ...
    try {
      if (existsSync(settingsFile)) {
        ...
        if (content._comment?.includes('qwenhub') || content.env?.ANTHROPIC_BASE_URL) {
